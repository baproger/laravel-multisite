...
@section('title', 'Редактирование: ' . $news->title_ru)
...
<form method="POST" action="{{ route('admin.news.update', $news->id) }}" enctype="multipart/form-data"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
                        ...
                        <div x-show="activeTab === '{{ $lang }}'" class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex">
                                    ...
                                    <input type="text" name="slug_{{ $lang }}"
                                           value="{{ old('slug_' . $lang, $news->{"slug_{$lang}"}) }}"
                                           {{ $lang === 'ru' ? 'required' : '' }}
                                           class="flex-1 px-3.5 py-2.5 border border-gray-300 rounded-r-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                </div>
                            </div>
                            <div>
                                ...
                                <textarea name="description_{{ $lang }}" rows="2"
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none">{{ old('description_' . $lang, $news->{"description_{$lang}"}) }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea name="content_{{ $lang }}" rows="12"
                                          {{ $lang === 'ru' ? 'required' : '' }}
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y">{{ old('content_' . $lang, $news->{"content_{$lang}"}) }}</textarea>
                            </div>
                            <details class="border border-gray-200 rounded-xl">
                                ...
                                <div class="px-4 pb-4 pt-3 space-y-3 border-t border-gray-100">
                                    <input type="text" name="meta_title_{{ $lang }}" value="{{ old('meta_title_'.$lang, $news->{"meta_title_{$lang}"}) }}"
                                    ...
                                    <textarea name="meta_description_{{ $lang }}" rows="2"
                                              ...
                                              placeholder="Meta Description...">{{ old('meta_description_'.$lang, $news->{"meta_description_{$lang}"}) }}</textarea>
                                </div>
                            </details>
                        </div>
                    @endforeach
        ...
        <div class="space-y-4">
            ...
            {{-- Изображение --}}
            <div class="bg-white border border-gray-200 rounded-xl p-5" x-data="imageUpload('{{ $news->image_url }}')">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-blue-400 transition-colors cursor-pointer"
                    ...
                    <template x-if="!preview">
                        <div>
                            ...
                            <p class="text-gray-400 text-xs">Нажмите для загрузки нового</p>
                        </div>
                    </template>
