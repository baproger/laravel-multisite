...
@section('title', 'Услуга: ' . $service->title_ru)
...
<form method="POST" action="{{ route('admin.services.update', $service->id) }}" enctype="multipart/form-data" x-data="{ activeTab: 'ru' }">
    @csrf @method('PUT')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
                ...
                <div class="flex border-b border-gray-200">
                    @foreach(['ru' => '🇷🇺 Русский', 'kz' => '🇰🇿 Қазақша', 'en' => '🇬🇧 English'] as $lang => $label)
                        <button type="button" @click="activeTab = '{{ $lang }}'"
                            ...
                            {{ $label }}
                            @if($service->{"title_{$lang}"})<span class="ml-1 w-1.5 h-1.5 bg-green-500 rounded-full inline-block"></span>@endif
                        </button>
                    @endforeach
                </div>
                <div class="p-5">
                        ...
                        <div x-show="activeTab === '{{ $lang }}'" class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="title_{{ $lang }}"
                                       value="{{ old('title_' . $lang, $service->{"title_{$lang}"}) }}"
                                       {{ $lang === 'ru' ? 'required' : '' }}
                            ...
                            <div>
                                ...
                                <div class="flex">
                                    ...
                                    <input type="text" name="slug_{{ $lang }}"
                                           value="{{ old('slug_' . $lang, $service->{"slug_{$lang}"}) }}"
                                           {{ $lang === 'ru' ? 'required' : '' }}
                            ...
                            <div>
                                ...
                                <textarea name="description_{{ $lang }}" rows="2"
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none">{{ old('description_' . $lang, $service->{"description_{$lang}"}) }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea name="content_{{ $lang }}" rows="12"
                                          {{ $lang === 'ru' ? 'required' : '' }}
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-y">{{ old('content_' . $lang, $service->{"content_{$lang}"}) }}</textarea>
                            </div>
                            <details class="border border-gray-200 rounded-xl">
                                ...
                                <div class="px-4 pb-4 pt-3 space-y-3 border-t border-gray-100">
                                    <input type="text" name="meta_title_{{ $lang }}"
                                           value="{{ old('meta_title_'.$lang, $service->{"meta_title_{$lang}"}) }}"
                                    ...
                                    <textarea name="meta_description_{{ $lang }}" rows="2"
                                              ...
                                              placeholder="Meta Description...">{{ old('meta_description_'.$lang, $service->{"meta_description_{$lang}"}) }}</textarea>
                                </div>
                            </details>
                        </div>
                    @endforeach
        ...
        <div class="space-y-4">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                            <option value="draft" {{ $service->status === 'draft' ? 'selected' : '' }}>Черновик</option>
                            <option value="published" {{ $service->status === 'published' ? 'selected' : '' }}>Опубликовано</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1.5">Иконка</label>
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-5" x-data="imageUpload('{{ $service->image ? asset('storage/' . $service->image) : '' }}')">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center hover:border-blue-400 transition-colors cursor-pointer"
                    ...
                    <template x-if="!preview">
                        <p class="text-gray-400 text-xs">Нажмите для загрузки</p>
                    </template>
            ...
            <div class="bg-white border border-red-100 rounded-xl p-4">
                ...
                <form method="POST" action="{{ route('admin.services.destroy', $service->id) }}"
                    ...
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full py-2 border border-red-300 text-red-600 text-sm rounded-lg hover:bg-red-50 transition-colors">Удалить услугу</button>
                </form>
            </div>
        </div>
    </div>
