...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Сотрудники компании</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-32">Должность</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($members as $member)
                    <tr class="{{ $member->trashed() ? 'bg-red-50/50' : 'hover:bg-gray-50' }} transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-3">
                                @if($member->photo_url ?? false)
                                    <img src="{{ $member->photo_url }}" alt="{{ $member->name_ru }}"
                                         class="w-10 h-10 rounded-xl object-cover">
                                @else
                                    <div class="w-10 h-10 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl flex items-center justify-center text-white text-sm font-semibold">
                                        {{ strtoupper(substr($member->name_ru, 0, 1)) }}
                                    </div>
                                @endif
                                <div>
                                    <p class="font-medium text-gray-900">{{ $member->name_ru }}</p>
                                    @if($member->email)
                        ...
                        <td class="px-4 py-3">
                            <span class="text-gray-600 text-sm">{{ $member->position_ru ?: '—' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium
                                {{ $member->status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                {{ $member->status === 'published' ? 'Опубликовано' : 'Черновик' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="text-gray-500 text-xs">{{ $member->sort_order }}</span>
                        </td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center text-gray-400">Сотрудники не найдены</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
