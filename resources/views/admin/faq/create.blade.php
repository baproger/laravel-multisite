...
@section('title', 'Новый вопрос FAQ')
...
<form method="POST" action="{{ route('admin.faq.store') }}" x-data="{ activeTab: 'ru' }">
    @csrf
                        ...
                        <div x-show="activeTab === '{{ $lang }}'" class="space-y-4">
                            <div>
                                ...
                                <textarea name="question_{{ $lang }}" rows="2"
                                          {{ $lang === 'ru' ? 'required' : '' }}
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none">{{ old('question_' . $lang) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">
                                ...
                                <textarea name="answer_{{ $lang }}" rows="5"
                                          {{ $lang === 'ru' ? 'required' : '' }}
                                          class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y">{{ old('answer_' . $lang) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">Категория {{ strtoupper($lang) }}</label>
                                <input type="text" name="category_{{ $lang }}" value="{{ old('category_' . $lang) }}"
